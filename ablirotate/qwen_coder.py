"""
Qwen Coder 30B (Qwen2.5-Coder-32B) specific toolkit.

Qwen2's MLP uses a SwiGLU / gated-linear-unit architecture::

    intermediate = act_fn(gate_proj(x)) * up_proj(x)   # (..., intermediate_size)
    output       = down_proj(intermediate)

Standard activation-based pruning targets a single weight matrix.  SwiGLU pruning
must coordinate changes across *three* matrices simultaneously:
...
Attention
---------
A ``register_forward_pre_hook`` on each ``o_proj`` captures the per-head output
vector (shape ``(batch, seq, n_heads × head_dim)``), reshaped to obtain a
per-head activation signal.  This mirrors the generic tracker's attention hook
but uses the exact head count for this model.
"""
...
#: Default configuration for Qwen2.5-Coder-32B (commonly called "Qwen Coder 30B").
QWEN_CODER_30B_CONFIG: Dict = {
    ...
    # Qwen2.5-Coder-32B attention geometry
...
class QwenCoderActivationTracker:
    """Track intermediate MLP and attention-head activations in a Qwen2 model.
    ...
    are most informative for SwiGLU-based models.

    Parameters
    ----------
    model : nn.Module
        A Qwen2-family causal language model (or any model with the same
        ``gate_proj`` / ``up_proj`` / ``down_proj`` / ``o_proj`` structure).
    config : dict, optional
        Architecture configuration.  Defaults to
        :data:`QWEN_CODER_30B_CONFIG`.
    threshold : float
    ...
    def __init__(
        ...
        self.model = model
        self.config = config if config is not None else QWEN_CODER_30B_CONFIG
    ...
    def __enter__(self) -> "QwenCoderActivationTracker":
        return self
...
class QwenCoderMlpPruner:
    """Prune SwiGLU MLP neurons across ``gate_proj``, ``up_proj``, and ``down_proj``.
    ...
    three projection matrices of each Qwen2 MLP block.

    Parameters
    ----------
    model : nn.Module
        The Qwen2-family model to prune (modified **in-place**).
    ...
    def prune(self, mode: str = "soft") -> Dict[str, int]:
        ...
        for mlp_path, stats in self.mlp_stats.items():
            ...
            if n_pruned == 0:
                results[mlp_path] = 0
                continue
            self._prune_swiglu(mlp_path, prune_mask, mode)
...
# ---------------------------------------------------------------------------
# SwiGLU-aware defragmenter
# ---------------------------------------------------------------------------

class QwenCoderDefragmenter:
    """Reorder and truncate SwiGLU MLP weight matrices by neuron activity order.
    ...
    * columns of ``down_proj`` (input neurons ← intermediate)
    ...
    ----------
    model : nn.Module
        The Qwen2-family model to defragment (modified **in-place**).
    mlp_stats : dict[str, ActivationStats]
    ...
    def defragment(self) -> Dict[str, Tuple[int, int]]:
        ...
        for mlp_path, stats in self.mlp_stats.items():
            ...
            order = order[:keep]
            self._permute_swiglu(mlp_path, order)
    ...
    def _permute_swiglu(self, mlp_path: str, order: torch.Tensor) -> None:
...
class QwenCoderPipeline:
    """Convenience end-to-end pipeline for Qwen Coder 30B specialisation.

    Parameters
    ----------
    model : nn.Module
        A Qwen2.5-Coder-32B (or compatible) causal language model.
    tokenizer
    ...
    def __init__(
        ...
        self.config = config if config is not None else QWEN_CODER_30B_CONFIG
        self.tracker = QwenCoderActivationTracker(
    ...
    def remove_hooks(self) -> None:
        """Detach all tracker hooks from the model."""
    ...
    def __enter__(self) -> "QwenCoderPipeline":
        return self
